...
def get_blocks(
    ...
    #print('id ' + text_type(usage_key))

    test = text_type(usage_key.course_key)
    course_key = usage_key.course_key  # CourseKey.from_string('course-v1:test+test+2020_T1')#course_id)
    student_username = username
    temp = {}
    for block in serializer.data:
        #print(block['id'])
        try:
            usage_key = UsageKey.from_string(block['id']).map_into_course(usage_key.course_key)
        ...
        try:
            ...
            # )))
        temp_list = []
        temp_data = {'link': request.get_host() + '/xblock/' + block['id']}
        for entry in history_entries:
            if 'student_answers' in str(entry.state):
                temp_list.append(entry.state)
        temp_data['records'] = temp_list
        temp_data['display_name'] = block['display_name']
        temp[block['id']] = temp_data

    return HttpResponse(json.dumps(temp))

    # try:
    #     usage_key = UsageKey.from_string(serializer.data[0]['id']).map_into_course(usage_key.course_key)
    # except (InvalidKeyError, AssertionError):
    #     return HttpResponse(escape(_(u'Invalid location.')))

    # user_state_client = DjangoXBlockUserStateClient()
    # try:
    #     history_entries = list(user_state_client.get_history_test(student_username, usage_key))
    # except DjangoXBlockUserStateClient.DoesNotExist:
    #     return HttpResponse(escape(_(u'User {username} has never accessed problem {location}').format(
    #         username=student_username,
    #         location=serializer.data[0]['id'],
    #         #key=usage_key
    #     )))
    ...
    # return render_to_response('courseware/test_history.html', context)
